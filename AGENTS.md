...
- **Phase 4** delivered the **product shell** layout ([`docs/prd/phase4.md`](docs/prd/phase4.md)); checklist **closed**. **Phase 5** wires **search + artist results** to **`searchArtists`** in [`src/shell/AppShell.tsx`](src/shell/AppShell.tsx) ([`docs/prd/phase5.md`](docs/prd/phase5.md)) — **manual validation** may still be open until the PRD checklist is marked. **Phase 6** wires **albums + album detail** via **`getArtistAlbums`** / **`getAlbum`** ([`docs/prd/phase6.md`](docs/prd/phase6.md)); Phase 5 **`ShellDevControls`** / **`shell/types.ts`** removed. PRD browser checklist for Phase 6 may remain until marked. **Phase 7:** [`docs/prd/phase7.md`](docs/prd/phase7.md) — accessibility + resilience implemented ([`src/App.tsx`](src/App.tsx), [`src/shell/AppShell.tsx`](src/shell/AppShell.tsx)). **Phase 8:** GitHub Pages via Actions ([`docs/prd/phase8.md`](docs/prd/phase8.md), [`.github/workflows/deploy-pages.yml`](.github/workflows/deploy-pages.yml)); **`vite.config.ts`** **`base: '/deezer-explorer/'`**. PRD production §§**1–4** pending after live deploy.
